                    ...
                    @if($invitation->status->value !== 'confirmed')
                        <form method="POST" action="{{ route('admin.events.invitations.approve', [$event, $invitation]) }}">
                            ...
                            <button type="submit" class="inline-flex items-center gap-2 rounded-md bg-emerald-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-emerald-500">
                                {{ __('invitation.moderation.approve') }}
                            </button>
                        </form>
                    @endif
                    @if($invitation->status->value !== 'cancelled')
                        <form method="POST" action="{{ route('admin.events.invitations.reject', [$event, $invitation]) }}">
                            @csrf
                            <button type="submit" class="inline-flex items-center gap-2 rounded-md bg-red-600 px-3 py-2 text-sm font-semibold text-white shadow-sm hover:bg-red-500">
                                {{ __('invitation.moderation.reject') }}
                            </button>
                        </form>
                    @endif
