<x-admin-layout>
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <h1 class="text-xl font-semibold text-white">{{ __('role.form.edit_title', ['name' => $role->name]) }}</h1>
        </div>
    </x-slot>

    <div class="rounded-lg bg-[var(--desert-surface)] p-4 sm:p-6">
        <div class="overflow-hidden rounded-lg bg-white shadow-sm">
            <form action="{{ route('admin.roles.update', $role) }}" method="POST" class="space-y-6 p-6">
                @csrf
                @method('PUT')
                @include('admin.roles._form', ['role' => $role, 'permissions' => $permissions, 'selected' => old('permissions', $selected)])
